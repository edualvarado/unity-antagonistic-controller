...
public class AntagonisticJoint : MonoBehaviour
{
    [Header("Joint")]
    ...
    private void FixedUpdate()
    {
        ...
        resultAngleQuaternion = ConfigurableJointExtensions.GetTargetRotationLocal(_jointAnt, kinematicAngleQuaternion, initialAngleQuaternion, this.transform);
        ...
        if (applyTorqueX)
        {
            ...
            torqueAppliedX = _AntPIDX.GetOutput(angleLowErrorX, angleHighErrorX, 0f, Time.fixedDeltaTime);
            if (printTorqueX)
            {
        ...
        if (applyTorqueY)
        {
            ...
            torqueAppliedY = _AntPIDY.GetOutput(angleHighErrorY, angleLowErrorY, 0f, Time.fixedDeltaTime);
            if (printTorqueY)
            {
        ...
        if (applyTorqueZ)
        {
            ...
            torqueAppliedZ = _AntPIDZ.GetOutput(angleLowErrorZ, angleHighErrorZ, 0f, Time.fixedDeltaTime);
            if (printTorqueZ)
            {
