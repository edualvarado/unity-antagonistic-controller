...
public class AntagonisticArmQuaternions : MonoBehaviour
{
    [Header("Quaternions")]
    ...
    {
        // Quaternions - NEW TODO
        Debug.Log("qTarget: " + target.rotation);
        Quaternion currentQ = sphereAnt.rotation * transform.localRotation;
        mimic.rotation = currentQ;

        Quaternion offsetQ = target.rotation * Quaternion.Inverse(currentQ);
        //mimic.rotation = offsetQ;

        //mimic.rotation = Quaternion.Euler(90f, 0, 0) * jointRotationQuaternion(_jointAnt);

        float angleQ = 0.0f;
        Vector3 axisQ = Vector3.zero;
        transform.rotation.ToAngleAxis(out angleQ, out axisQ);

        Debug.Log("angleQ: " + angleQ + " - axisQ: " + axisQ);
        ...
        Debug.Log("currentAngleQuaternion: " + Quaternion.Euler(currentAngle));
    ...
    Quaternion jointRotationQuaternion(ConfigurableJoint joint)
    {
        Quaternion jointBasis = Quaternion.LookRotation(joint.secondaryAxis, Vector3.Cross(joint.axis, joint.secondaryAxis));
        Quaternion jointBasisInverse = Quaternion.Inverse(jointBasis);
        return (jointBasisInverse * Quaternion.Inverse(joint.connectedBody.rotation) * joint.GetComponent<Rigidbody>().transform.rotation * jointBasis);
    }
}
